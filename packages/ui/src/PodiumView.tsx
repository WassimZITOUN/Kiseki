...
export function PodiumView({ members }: Props) {
  if (members.length === 0) return null;

  // Reorder: [2nd, 1st, 3rd] for visual layout
  const display: { member: PodiumMember; configIndex: number }[] = [];

  if (members[0]) display.push({ member: members[0], configIndex: 0 }); // 1st
  if (members[1]) display.push({ member: members[1], configIndex: 1 }); // 2nd
  if (members[2]) display.push({ member: members[2], configIndex: 2 }); // 3rd

  // Visual order: 2nd - 1st - 3rd
  const sorted = [...display].sort(
    (a, b) => PODIUM_CONFIG[a.configIndex].order - PODIUM_CONFIG[b.configIndex].order
  );
  ...
  return (
    ...
    >
      {sorted.map(({ member, configIndex }, i) => (
        <PodiumSlot
          key={member.userId}
          member={member}
          config={PODIUM_CONFIG[configIndex]}
          delay={configIndex * 200}
        />
      ))}
    </View>
  );
