    ...
    return (<AuroraScreenWrapper>
      <ScrollView contentContainerStyle={{ flexGrow: 1, padding: spacing.lg }}>
        <GlassCard style={{
            ...
            <KAvatar uri={profile?.avatar_url} name={profile?.display_name ?? profile?.username} size={120}/>
          </GlassCard>

        <GlassCard style={{ marginBottom: spacing.md }}>
          <View style={{ marginBottom: spacing.md }}>
            <KText variant="caption" color={colors.textMuted} style={{ marginBottom: spacing.xs }}>
              Nom affiche
            </KText>
        ...
        {isWeb && (<GlassCard style={{ marginBottom: spacing.md, borderRadius: radii.xl }}>
            <KText variant="h3" style={{ marginBottom: spacing.sm }}>
              Apparence
            </KText>
            <KButton title={darkModeEnabled ? "Desactiver le dark mode" : "Activer le dark mode"} onPress={toggleDarkMode} variant="glass"/>
          </GlassCard>)}

        <GlassCard style={{
            borderRadius: radii.xl,
        }}>
          <KButton title="Modifier le profil" onPress={onNavigateEdit} style={{ marginBottom: spacing.sm }}/>

          <KButton title="Se deconnecter" onPress={signOut} variant="glass"/>
        </GlassCard>
      </ScrollView>
    </AuroraScreenWrapper>);
}
