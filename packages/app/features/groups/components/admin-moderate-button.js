...
export function AdminModerateButton({ groupId, dailyQuestionId, isAdmin, disabled, onReplaced, }) {
    ...
    return (<>
      <TouchableOpacity onPress={() => setConfirmVisible(true)} disabled={disabled} activeOpacity={0.7} style={{
            flexDirection: "row",
            alignItems: "center",
            alignSelf: "center",
        ...
        {/* Blur background */}
        {!isWeb && (<View style={[StyleSheet.absoluteFill, { borderRadius: 20, overflow: "hidden" }]}>
        ...
        <Feather name="refresh-cw" size={14} color={disabled ? colors.textMuted : colors.textSecondary} style={{ marginRight: spacing.xs }}/>
        <KText variant="caption" color={disabled ? colors.textMuted : colors.textSecondary}>
          Remplacer
        </KText>
      </TouchableOpacity>
      ...
      <GlassModal visible={confirmVisible} onClose={() => {
        ...
        <View style={{
          ...
          <KButton title="Annuler" onPress={() => {
            setConfirmVisible(false);
            setError(null);
        }} variant="glass" disabled={replacing} style={{ minWidth: 110, marginRight: spacing.sm }}/>
          <KButton title="Remplacer" onPress={handleReplace} loading={replacing} disabled={replacing} style={{ minWidth: 110 }}/>
        </View>
      </GlassModal>
    </>);
}
