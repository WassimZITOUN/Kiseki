    ...
    return (<GlassBottomSheet visible={visible} onClose={onClose}>
      ...
      {sortedGroups.map(([targetId, targetVotes], groupIndex) => {
            ...
            <View style={{
              ...
              <KAvatar uri={target.avatar_url} name={targetName} size={28}/>
              <KText color={colors.violet[500]} variant="body" style={{
              ...
              <View style={{
                ...
                <KText variant="caption" color={colors.violet[300]} style={{ fontWeight: "600" }}>
            ...
            {targetVotes.map((vote, voteIndex) => {
                    ...
                    return (<View key={`${vote.voter.id}-${voteIndex}`} style={{
                            flexDirection: "row",
                  ...
                  <KAvatar uri={vote.voter.avatar_url} name={voterName} size={24}/>
                  <View style={{ marginLeft: spacing.xs, flex: 1 }}>
                    <KText variant="bodySmall" color={colors.violet[300]} style={{ fontWeight: "500" }}>
                      {voterName}
                    </KText>
                    {vote.context_note && (<KText variant="caption" color={colors.textPrimary} style={{ fontStyle: "italic", marginTop: 2 }}>
                        "{vote.context_note}"
                      </KText>)}
                  </View>
                </View>);
      ...
      {votes.length === 0 && (<KText variant="body" color={colors.textMuted} style={{ textAlign: "center", paddingVertical: spacing.lg }}>
          Aucun vote
        </KText>)}
    </GlassBottomSheet>);
}
