...
import type { QuestionIntensity } from "@my-app/types";
...
export function SubmitQuestionCard({ groupId, slotId, onSubmitted }: Props) {
  const [questionText, setQuestionText] = useState("");
  const [intensity, setIntensity] = useState<QuestionIntensity>("normal");
  const [mode, setMode] = useState<"custom" | "bank">("custom");
  ...
  const isValid =
    mode === "bank" || (trimmed.length >= 10 && trimmed.length <= 200);
        ...
        <GlassCard>
          ...
          <View style={{ flexDirection: "row", marginBottom: spacing.md }}>
            ...
            >
              <KText
                variant="body"
                color={mode === "custom" ? colors.textPrimary : colors.textSecondary}
              >
                Ma question
              </KText>
            </TouchableOpacity>
            <TouchableOpacity
              ...
              style={{
                ...
                borderWidth: 1,
                borderColor:
                  mode === "bank" ? colors.success : "rgba(255, 255, 255, 0.1)",
              }}
            >
              <KText
                variant="body"
          ...
          {mode === "custom" ? (
            <>
              <KInput
                label="Ta question"
                value={questionText}
                onChangeText={(t) => setQuestionText(t.slice(0, 200))}
              ...
              <KText
                ...
                style={{ textAlign: "right", marginBottom: spacing.md }}
              >
          ...
          <View style={{ flexDirection: "row", marginBottom: spacing.lg }}>
            ...
            >
              <KText
                variant="body"
                color={intensity === "normal" ? colors.textPrimary : colors.textSecondary}
              >
                Normal
              </KText>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => setIntensity("epice")}
              activeOpacity={0.7}
              style={{
                flex: 1,
                paddingVertical: spacing.sm,
