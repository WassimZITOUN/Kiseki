...
export function CreateGroupScreen({ onGroupCreated, onBack }) {
    ...
    if (inviteCode && createdGroupId) {
        ...
        <View style={{
          ...
          <Animated.View entering={FadeIn.duration(400)}>
            ...
            <KButton title={copied ? "Copie !" : "Copier le code"} onPress={handleCopy} variant="glass" style={{ marginBottom: spacing.md }}/>

            <KButton title="Voir le groupe" onPress={() => onGroupCreated?.(createdGroupId)}/>
          </Animated.View>
        </View>
      </AuroraScreenWrapper>);
    }
    return (<AuroraScreenWrapper>
      <View style={{
        ...
        <KText variant="h1" style={{ marginBottom: spacing.lg }}>
          Creer un groupe
        </KText>

        <GlassCard style={{ marginBottom: spacing.lg }}>
          <KInput label="Nom du groupe" value={name} onChangeText={setName} placeholder="Ex: La team Nasdas" maxLength={GROUP_NAME_MAX_LENGTH} containerStyle={{ marginBottom: spacing.md }}/>

          <KInput label="Nombre max de membres (2-12)" value={maxMembers} onChangeText={setMaxMembers} keyboardType="numeric"/>
        </GlassCard>

        <ErrorBanner message={error}/>

        <KButton title="Creer le groupe" onPress={handleCreate} loading={loading}/>
      </View>
    </AuroraScreenWrapper>);
}
