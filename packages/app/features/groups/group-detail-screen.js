...
export function GroupDetailScreen({ groupId, onLeft, onBack }) {
    ...
    const renderInviteCode = () => {
        if (!group)
            return null;
        return (<TouchableOpacity onPress={handleCopyCode} style={{
        ...
        <GlassCard style={{
          ...
          <KText variant="caption" color={codeCopied ? colors.success : colors.textSecondary} style={{ fontWeight: "500" }}>
            {codeCopied
                ? "Code copie !"
    ...
    const headerRightAction = (<TouchableOpacity onPress={() => setMenuVisible(true)} activeOpacity={0.8} style={{
            width: 44,
            height: 44,
            borderRadius: 22,
            shadowColor: "#A29BFE",
    ...
    if (question && question.status === "revealed") {
        ...
        return (<AuroraScreenWrapper>
        <KHeader title={group.name} onBack={onBack} rightAction={headerRightAction}/>

        <ScrollView contentContainerStyle={{ paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
          {/* Title */}
          <Animated.View entering={FadeIn.duration(600)}>
            <KText variant="questionLarge" style={{
                textAlign: "center",
            ...
            <KText variant="bodySmall" color={colors.textSecondary} style={{
                textAlign: "center",
          ...
          {allResults.length > 0 ? (<>
              ...
              {/* Rest of results */}
              {restResults.length > 0 && (<View style={{ paddingHorizontal: spacing.md, marginTop: spacing.md }}>
                  {restResults.map((item, i) => (<ResultCard key={item.userId} item={item} rank={i + 4} delay={600 + i * 100}/>))}
                </View>)}

              {/* Vote details button */}
              <View style={{ paddingHorizontal: spacing.md, marginTop: spacing.lg }}>
                <KButton title="Detail des votes" onPress={() => setVoteDetailsVisible(true)} variant="glass"/>
              </View>

              {/* Share button (mobile only) */}
              {!isWeb && (<View style={{ paddingHorizontal: spacing.md, marginTop: spacing.sm }}>
                <KButton title="Partager" onPress={() => setShareSheetVisible(true)} variant="glass" leftIcon={<Feather name="share-2" size={16} color={colors.textPrimary}/>}/>
              </View>)}

              {/* Submit question card (State D) */}
              {shouldShowSubmitCard && myNextSlot?.slot_id && (<SubmitQuestionCard groupId={groupId} slotId={myNextSlot.slot_id} onSubmitted={fetchData}/>)}
          ...
          {/* Tomorrow teaser */}
          {tomorrowCountdown !== "" && (<View style={{ paddingHorizontal: spacing.md, marginTop: spacing.lg }}>
        ...
        <GroupMenu visible={menuVisible} onClose={() => setMenuVisible(false)} members={members} group={group} leaving={leaving} onLeave={handleLeave} onCopyCode={handleCopyCode} codeCopied={codeCopied} currentUserId={currentUserId} onRemoveMember={handleRemoveMember}/>

        <VoteDetailsSheet visible={voteDetailsVisible} onClose={() => setVoteDetailsVisible(false)} votes={questionResults?.votes ?? []} question={question.question}/>

        {!isWeb && (<ShareResultSheet visible={shareSheetVisible} onClose={() => setShareSheetVisible(false)} question={question.question} winnerName={allResults[0]?.name ?? ""} winnerAvatarUri={allResults[0]?.avatarUri} winnerVoteCount={allResults[0]?.voteCount ?? 0} groupName={group.name}/>)}
      </AuroraScreenWrapper>);
    }
    ...
    if (question === null) {
        ...
        <ScrollView contentContainerStyle={{
          ...
          {shouldShowWaitingForOtherSubmission && (<Animated.View entering={FadeIn.duration(300)} style={{ marginTop: spacing.md }}>
          ...
          {!shouldShowSubmitCard && !shouldShowWaitingForOtherSubmission && (<Animated.View entering={FadeIn.duration(250)}>
              <GlassCard style={{ alignItems: "center" }}>
                <KText variant="bodySmall" color={colors.textSecondary} style={{ textAlign: "center" }}>
    ...
    if (myVote) {
        ...
        <ScrollView contentContainerStyle={{
          ...
          {showVoteReview ? (<Animated.View entering={FadeIn.duration(300)} style={{ alignItems: "center", width: "100%" }}>
              <GlassCard style={{
                    width: "100%",
                    maxWidth: 340,
                    marginBottom: spacing.lg,
                }}>
                <KText variant="h3" style={{
                    textAlign: "center",
                ...
                {myVote.context_note && (<KText variant="bodySmall" color={colors.textMuted} style={{
                        textAlign: "center",
                        fontStyle: "italic",
              ...
              <KButton title="Fermer" onPress={() => setShowVoteReview(false)} variant="ghost"/>
            </Animated.View>) : (<Animated.View entering={SlideInUp.duration(400)} style={{ alignItems: "center" }}>
              <GlassCard style={{
                ...
                <KButton title="Revoir mon vote" onPress={() => setShowVoteReview(true)} variant="glass" style={{ marginTop: spacing.xl }}/>
              </GlassCard>
            </Animated.View>)}

          {/* Submit question card (State B) */}
          {shouldShowSubmitCard && myNextSlot?.slot_id && (<View style={{ width: "100%" }}>
              <SubmitQuestionCard groupId={groupId} slotId={myNextSlot.slot_id} onSubmitted={fetchData}/>
            </View>)}
        </ScrollView>
    ...
    return (<AuroraScreenWrapper>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 20 }} showsVerticalScrollIndicator={false} style={{ flex: 1 }}>
        <GlassCard style={{
            marginHorizontal: spacing.md,
            marginBottom: spacing.md,
        }}>
          <QuestionHeader question={question.question} subtitle="Question du jour"/>

          {/* Admin moderate button (State A) */}
          {question && (<AdminModerateButton groupId={groupId} dailyQuestionId={question.id} isAdmin={isCurrentUserAdmin} disabled={adminReplacedToday} onReplaced={fetchData}/>)}
        </GlassCard>

        <VoteGrid members={otherMembers} selectedUserId={selectedUserId} onSelectMember={handleMemberTap} disabled={!!myVote}/>
      ...
      <VoteConfirmModal visible={voteModalVisible} targetName={selectedUserId ? getTargetName(selectedUserId) : ""} contextNote={contextNote} onChangeContextNote={setContextNote} onConfirm={handleSubmitVote} onCancel={() => {
            setVoteModalVisible(false);
            setSelectedUserId(null);
            setContextNote("");
